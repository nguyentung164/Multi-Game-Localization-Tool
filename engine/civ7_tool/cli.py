...
from .deploy import deploy_to_game
from .editor import list_localization, update_localization_entry
from .inventory import export_game_files, inspect_localization
from .qa import run_qa
from .search import search_localization
from .state import validate_state
from .sync import apply_sync, preview_sync, restore_backup
from .translate import DEFAULT_MODELS, TranslationConfig, translate_localization
from .types import (
...
class JsonlEmitter:
    ...
    def _artifact_root(self) -> Path:
        ...
        return Path(tempfile.gettempdir()) / "civ7-tool-events"
...
def _load_glossary(config: Mapping[str, Any]) -> Mapping[str, str]:
    ...
    if inline is not None:
        if not isinstance(inline, Mapping):
            raise ValidationError("config.glossary phải là JSON object")
...
def _dispatch(
    ...
    if command == "sync-apply":
        ...
        if backup_root is None:
            ...
            backup_root = target.parent / ".civ7-tool-backups"
        return apply_sync(
            _path(config, "englishDir"),  # type: ignore[arg-type]
    ...
    if command == "translate":
        ...
        if cache_path is None:
            cache_path = target.parent / ".civ7-tool" / "translation-cache.json"
    ...
    if command in {"deploy-preview", "deploy-apply"}:
        ...
        if not dry_run and backup_root is None:
            ...
            backup_root = mod.parent / ".civ7-tool-deploy-backups"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if isinstance(staging, str) and staging.strip():
        artifact_dir = Path(staging).expanduser() / ".civ7-tool" / "engine-events"
    try:
    ...
    emitter.emit("started", command, {"command": command})
    try:
        ...
        emitter.emit("error", command, {"message": str(error), "code": "CANCELLED"})
        emitter.emit("completed", command, {"status": "cancelled"})
        return 130
    except ValidationError as error:
        emitter.emit(
            "error", command, {"message": str(error), "code": "VALIDATION_ERROR"}
        )
        emitter.emit("completed", command, {"status": "failed"})
        return 2
    except Civ7ToolError as error:
        emitter.emit(
            "error", command, {"message": str(error), "code": "OPERATION_FAILED"}
        )
        emitter.emit("completed", command, {"status": "failed"})
        return 1
